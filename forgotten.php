	...
	$(document).ready(function() {
		...
		$("#submit").click(function () {
				...
				$.ajax({
					...
					data: {'email':email, newpass:pass, confirmpass:conpass},
					success: function (msg) {
						alert('success'+msg);

						if (msg == 'db updated') {
							$('.message').html('<br><span xclass="badge badge-success">Password changed</span>');

						}
						else {
							$('.message').html('<br><span xclass="badge badge-danger">' + msg + '</span>');
							alert('error '+JSON.stringify(msg));
						}
					},
					error: function (msg) {
						alert('error'+JSON.stringfy(msg));
					}
				});
				return false;
			});
